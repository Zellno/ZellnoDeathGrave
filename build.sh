...
SOURCE_DIR="$PROJECT_DIR/source/ZellnoDeathGrave"
BUILD_ROOT="$PROJECT_DIR/build"
BUILD_MOD="$BUILD_ROOT/@ZellnoDeathGrave"
...
SERVER_MOD="$HOME/dayz/server/@ZellnoDeathGrave"
SERVER_KEYS="$HOME/dayz/server/keys"
INSTALL_BACKUPS="$PROJECT_DIR/backups/install"

EXPECTED_BUILD_MOD="$PROJECT_DIR/build/@ZellnoDeathGrave"
...
wine "$FILEBANK" \
    -property prefix=ZellnoDeathGrave \
    -dst "$(winepath -w "$BUILD_ADDONS")" \
    "$(winepath -w "$SOURCE_DIR")"

PBO="$BUILD_ADDONS/ZellnoDeathGrave.pbo"
...
if [ -e "$SERVER_MOD" ]; then
    ...
    BACKUP="$INSTALL_BACKUPS/@ZellnoDeathGrave-$TIMESTAMP"
