...
PROJECT = HOME / "dayz/modding/ZellnoDeathGrave"
BUILD_MOD = PROJECT / "build/@ZellnoDeathGrave"

SERVER = HOME / "dayz/server"
SERVER_MOD = SERVER / "@ZellnoDeathGrave"
SERVER_KEY = SERVER / "keys/Zellno.bikey"

CLIENT = HOME / ".local/share/Steam/steamapps/common/DayZ"
CLIENT_MOD = CLIENT / "@ZellnoDeathGrave"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Instala Zellno Death Grave de forma reversível."
        )
    )
    parser.add_argument(
        "--apply",
    ...
    require_file(
        BUILD_MOD / "addons/ZellnoDeathGrave.pbo"
    )
    require_file(
        BUILD_MOD
        / "addons/ZellnoDeathGrave.pbo.Zellno.bisign"
    )
    ...
    print("=" * 76)
    print(" Zellno Death Grave — instalação local")
    print("=" * 76)
    print()
    ...
    backup = (
        BACKUP_ROOT
        / f"install-zellno-death-grave-{timestamp}"
    )
    ...
    if server_existed:
        if SERVER_MOD.is_symlink():
            ...
            shutil.copytree(
                SERVER_MOD,
                backup / "@ZellnoDeathGrave.server",
            )
        else:
            shutil.copy2(
                SERVER_MOD,
    ...
    if client_existed:
        if CLIENT_MOD.is_symlink():
            ...
            shutil.copytree(
                CLIENT_MOD,
                backup / "@ZellnoDeathGrave.client",
            )
        else:
            shutil.copy2(
                CLIENT_MOD,
    ...
    stage = Path(
        tempfile.mkdtemp(
            prefix=".ZellnoDeathGrave-stage-",
            dir=SERVER,
        )
    )

    try:
        staged_mod = stage / "@ZellnoDeathGrave"
        ...
        server_backup = backup / "@ZellnoDeathGrave.server"
        ...
        client_backup = backup / "@ZellnoDeathGrave.client"
